...
# This is the definition of a RideCymbal
class RideCymbal(MusicianStructured):

    # Initialization of a RideCymbal. Initializes RideCymbal specific values and
    ...
    def __init__(self, energy=50, complexity=50, time = (4,4), key = ('B', 'major')):
        ...
        self.instrument = 'ride cymbal'
        self._my_tone=DrumKit["Ride Cymbal 1"]
    ...
    # For a RideCymbal, this is done by playing mainly on-beat notes (as able)
    ...
    #   RideCymbal, a note should run into the next note and be no more than
    ...
    def _onbeat(self):
        ...
        for x in range(self._time[0]):
            ...
            if self._notes >= 2*(self._time[0]-x):
    ...
    def _offbeat(self):
        ...
        while self._notes > 0 and y < 20:
            y += 1
            # Randomly add some eighth notes, if needed
            for x in range(self._time[0]):
...
# Returns the construtor for the RideCymbal		
def Musician():
    return RideCymbal()
